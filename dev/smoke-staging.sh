#!/usr/bin/env bash
# ============================================================
# NexoGestão — Smoke Test de Staging
# ============================================================
# Executa testes básicos para validar se a aplicação está online
# e funcional após o deploy em staging.
#
# Uso:
#   ./dev/smoke-staging.sh [API_BASE_URL] [WEB_BASE_URL]
#
# Variáveis de ambiente:
#   API_BASE: URL base da API (ex: https://api-staging.seudominio.com.br)
#   WEB_BASE: URL base do Web (ex: https://app-staging.seudominio.com.br)
#   ADMIN_EMAIL: E-mail de um usuário admin para login
#   ADMIN_PASSWORD: Senha do usuário admin
#
# Este script é chamado automaticamente pelo deploy-staging.sh.
# ============================================================
set -euo pipefail

# ─── Configuração ─────────────────────────────────────────────
API_BASE="${API_BASE:-http://localhost:3001}" # Default para localhost se não setado
WEB_BASE="${WEB_BASE:-http://localhost:3000}" # Default para localhost se não setado
...
require_command() {
  if ! command -v "$1" &>/dev/null; then
    log_fail "Comando $1 não encontrado. Instale-o e tente novamente."
    exit 1
  fi
}
...
# ─── Testes ───────────────────────────────────────────────────────────────────
log_section "Iniciando Smoke Test de Staging"
...
if [[ "$FAILED" -eq 0 ]]; then
  log_ok "Todos os testes de smoke passaram! Aplicação de staging online e funcional."
  exit 0
else
  log_fail "$FAILED teste(s) de smoke falharam. Verifique os logs acima."
  exit 1
fi
