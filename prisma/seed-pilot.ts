...
async function upsertCustomer(orgId: string, customer: CustomerSeed) {
  const existing = await prisma.customer.findFirst({
    where: { orgId, email: customer.email.toLowerCase() },
  })
  ...
  if (found) {
    return prisma.serviceOrder.update({
      ...
      data: {
        ...
        executionStartedAt:
          params.status === ServiceOrderStatus.IN_PROGRESS || params.status === ServiceOrderStatus.DONE
            ? atHour(new Date(), -1, 9)
            : null,
        executionEndedAt: params.status === ServiceOrderStatus.DONE ? atHour(new Date(), -1, 11) : null,
      },
    })
  }
    ...
    data: {
      ...
      executionStartedAt:
        params.status === ServiceOrderStatus.IN_PROGRESS || params.status === ServiceOrderStatus.DONE
          ? atHour(new Date(), -1, 9)
          : undefined,
      executionEndedAt: params.status === ServiceOrderStatus.DONE ? atHour(new Date(), -1, 11) : undefined,
    },
  })
}
...
async function main() {
  ...
  const savedUsers = await Promise.all(users.map((user) => upsertUser(org.id, user)))
  const [adminUser, operatorUser, financeUser] = savedUsers

  const operatorPerson = await prisma.person.findUnique({ where: { userId: operatorUser.id } })
  ...
  const customers = await Promise.all(customerSeeds.map((customer) => upsertCustomer(org.id, customer)))

  const appointments = await Promise.all([
    upsertAppointment({
      orgId: org.id,
      customerId: customers[0].id,
      title: 'Visita técnica mensal - elétrica áreas comuns',
      description: 'Checklist de painéis e iluminação do condomínio.',
      startsAt: atHour(now, 1, 8, 30),
      endsAt: atHour(now, 1, 10, 30),
      status: AppointmentStatus.CONFIRMED,
      notes: 'Levar EPI completo e validar acesso na portaria.',
    }),
    upsertAppointment({
      orgId: org.id,
      customerId: customers[1].id,
      title: 'Inspeção de ar-condicionado - recepção',
      description: 'Verificar ruído e eficiência térmica.',
      startsAt: atHour(now, 2, 14, 0),
      endsAt: atHour(now, 2, 15, 30),
      status: AppointmentStatus.SCHEDULED,
      notes: 'Equipamento modelo split 36.000 BTUs.',
    }),
    upsertAppointment({
      orgId: org.id,
      customerId: customers[2].id,
      title: 'Manutenção corretiva - coifa industrial',
      description: 'Troca de contatores e limpeza de dutos.',
      startsAt: atHour(now, -2, 21, 0),
      endsAt: atHour(now, -2, 23, 0),
      status: AppointmentStatus.DONE,
      notes: 'Serviço concluído dentro do prazo.',
    }),
    upsertAppointment({
      orgId: org.id,
      customerId: customers[3].id,
      title: 'Revisão preventiva hidráulica',
      description: 'Inspeção de banheiros e caixas de descarga.',
      startsAt: atHour(now, 3, 9, 0),
      endsAt: atHour(now, 3, 10, 0),
      status: AppointmentStatus.CONFIRMED,
      notes: 'Acompanhamento da coordenação escolar.',
    }),
    upsertAppointment({
      orgId: org.id,
      customerId: customers[4].id,
      title: 'Diagnóstico de rede interna',
      description: 'Pontos sem conectividade no estoque.',
      startsAt: atHour(now, 0, 16, 0),
      endsAt: atHour(now, 0, 17, 30),
      status: AppointmentStatus.NO_SHOW,
      notes: 'Responsável da loja não estava presente para liberar acesso.',
    }),
  ])
  ...
  const serviceOrders = await Promise.all([
    upsertServiceOrder({
      ...
      description: 'Reaperto em barramentos e substituição de 12 lâmpadas LED.',
      amountCents: 185000,
    ...
    upsertServiceOrder({
      ...
      description: 'Limpeza completa e troca de filtro HEPA.',
      amountCents: 124000,
    ...
    upsertServiceOrder({
      ...
      description: 'Troca de contatores, testes e emissão de relatório técnico.',
      amountCents: 98000,
      dueDate: atHour(now, -1, 18, 0),
      status: ServiceOrderStatus.DONE,
      outcomeSummary: 'Equipamento liberado para uso e monitoramento semanal.',
    }),
    upsertServiceOrder({
      orgId: org.id,
      customerId: customers[3].id,
      ...
      description: 'Substituição de válvulas e vedação de 4 pontos de vazamento.',
      amountCents: 76000,
    ...
    upsertServiceOrder({
      ...
      description: 'Passagem de novo cabeamento CAT6 e troca de switch principal.',
      amountCents: 212000,
  ...
  const chargeDueFuture = atHour(now, 10, 12, 0)
  const chargeDueOverdue = atHour(now, -5, 12, 0)

  const charge1 = await prisma.charge.upsert({
    where: { orgId_serviceOrderId: { orgId: org.id, serviceOrderId: serviceOrders[0].id } },
    update: {
      customerId: customers[0].id,
      amountCents: 185000,
      dueDate: chargeDueFuture,
      status: ChargeStatus.PENDING,
      title: 'Cobrança OS-001',
  ...
  const charge2 = await prisma.charge.upsert({
    where: { orgId_serviceOrderId: { orgId: org.id, serviceOrderId: serviceOrders[2].id } },
    update: {
      customerId: customers[2].id,
      amountCents: 98000,
  ...
  await prisma.payment.upsert({
    where: { orgId_chargeId: { orgId: org.id, chargeId: charge2.id } },
    update: {
      amountCents: 98000,
  ...
  const standaloneCharges = [
    {
      customerId: customers[1].id,
      amountCents: 124000,
      dueDate: chargeDueOverdue,
      status: ChargeStatus.OVERDUE,
      title: 'Cobrança avulsa clínica',
    ...
    },
    {
      ...
      status: ChargeStatus.PENDING,
      title: 'Cobrança sinal reestruturação de rede',
      notes: null,
    },
  ]

  for (const seed of standaloneCharges) {
  ...
  for (const invoice of invoiceSeeds) {
    await prisma.invoice.upsert({
      where: { orgId_number: { orgId: org.id, number: invoice.number } },
      update: invoice,
      create: {
        orgId: org.id,
        ...invoice,
